...
effectdict = {'all': ['all enemies receive damage', {'condition': False, 'turns': 0}],
              ...
              'calm': ['player receives 50% damage', {'condition': True, 'turns': 1}],
              'wrath': ['player deals 200% damage, player receives 200% damage', {'condition': True, 'turns': 1}],
              'exit stance': ['player deals 100% damage, player receives 100% damage', {'condition': True, 'turns': 1}],
...
enemycardsdict = {'snail strike': Card(6, 0, 0, 'Snail Strike', []),
                  ...
                  'thrash': Card(7, 2, 0, 'Snail Thrash', []),
                  'bellow': Card(3, 3, 0, 'Snail Bellow', []),
...
class_cards = {'tank': tankcardsdict, 'mage': magecardsdict,
               'rogue': roguecardsdict, 'enemy': enemycardsdict}
...
def not_enough_mana(player, card):
    ...
    print("It requires {} mana to play that card, you have {} mana".format(
        card_obj.mana, player.mana))
    print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
    print("")
    time.sleep(2)
    return
...
def get_class_cards():
    class_cards = {'tank': get_tankcardsdict(
    ), 'mage': get_magecardsdict(), 'rogue': get_roguecardsdict()}
    return class_cards
...
def get_description(snail):
    ...
    print(divider)
    print("{} Health / Level {} / drops {} coins on defeat".format(snail.hp,
          snail.powerlevel, snail.coins))
...
def change_stance(actor):
    stances = {'calm': calm, 'wrath': wrath, 'exit stance': exit_stance}
    new_dict = {}
    stance_wanted = None
    for condition in actor.conditions:
        if condition in stances:
            stance_wanted = condition
        else:
            new_dict[condition] = actor.conditions[condition]
    if stance_wanted is not None:
        new_dict[stance_wanted] = 1
        stances[stance_wanted](actor)
