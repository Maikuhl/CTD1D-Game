...
def get_player_status(player):
  print("{}: {} Mana / {} Health / {} Block / conditions {}".format(player.name, player.mana, player.hp, player.block, player.conditions))
  return
...
def print_enemies(enemies_list):
  ...
  for enemy in enemies_list:
      print("{} / {} Health / {} block / conditions: {}".format(enemy.name, enemy.hp, enemy.block, enemy.conditions))
...
def print_attack(actor, target, card, damage):
  ...
  sleep(0.5)
  print("{} attacks {} for {} damage".format(actor.name, target.name, damage))
  sleep(0.5)
  return
