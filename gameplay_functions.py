...
def choose_card_reward(player):
    ...
    card_options = [card_list[randint(0, len(card_list)-1)], card_list[randint(
        0, len(card_list)-1)], card_list[randint(0, len(card_list)-1)]]
    while True:
...
def deal_damage(actor, enemy, card):
    damage_done = max(((actor.damage + card.damage)*actor.damagemult)
                      * enemy.takedamagemult - enemy.block, 0)
    enemy.block = max(enemy.block - ((actor.damage + card.damage)
                      * actor.damagemult)*enemy.takedamagemult, 0)
...
def update_conditions(actor):
    new_conditions = {}
    for condition in actor.conditions.keys():
        if condition == 'poison':
            actor.hp -= 3*actor.takedamagemult
        if actor.conditions[condition] == 0:
            pass
        else:
            new_conditions[condition] = actor.conditions[condition] - 1
