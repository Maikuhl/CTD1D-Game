...
> This game requires the following libraries installed (pre-loaded in python3):
...
**Methods:**
     ...
     -	Define_effect(effect_ls): returns a list of effects that should be applied when the card is played. Each effect is defined as a dictionary: [{'target':target, 'action': action, 'value': value}, ...]
     
     -	description(): Describes the card, displays mana requirement, damage, block, and its effects.
          -	Returns a string of the descriptions
          
     -	Get_effect_tags():
          -	Returns a dictionary of effects
...
### Functions that modify the attributes of an object:
     -	Get_description(snail):
          -	Takes in a snail object, and returns a formatted string with snail stats.

     -	Flex(actor): 
          -	Increases the damage attribute of an object by 1 (int)

     -	Twice(actor):
          -	Doubles the damage received by an object

     -	Calm(actor):
          -	 Halves the damage received by the player object

     -	Wrath(actor):
          -	Doubles the damage dealt and damage received by the player object

     -	Exit_stance():
          -	Cleanse all buffs and debuffs of the player

     -	Vulnerable():
          -	Enemy receives 150% damage 

     -	Weak(actor):
          -	Halves the damage dealt by the enemy object

     -	Poison():
          -	Enemy receives 3 damage for 5 turns

     -	Snail_ritual():
          -	Player deals 1 (int) additional damage

     -	Snail_grow():
          -	Player deals 1 (int) additional damage

     -	Snail_weak():
          -	Enemy deals 75% damage

     -	Drain(actor):
          -	Takes in an object and returns modified attribute

     -	Gain_mana(actor):
          -	Takes in an object and returns modified attribute

     -	Copy(actor): 
          -	Takes in an object and appends the chosen card to be copied into the hand list.

     -	Snail_spit(actor): 
          -	Appends string “lick” to the graveyard list
...
## Gameplay Functions 
     ...
     Story Functions
          -	Def Chapter_n():
          -    It presents storyline every chapter
